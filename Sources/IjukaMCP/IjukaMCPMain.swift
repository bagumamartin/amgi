...
/// ijuka-mcp — Model Context Protocol server over Ijuka's Rust engine.
///
...
struct IjukaMCPMain {
    ...
    static func main() async {
        ...
        let log = Logger(label: "ijuka-mcp")
        ...
        do {
            ...
            // collection lock while idle, and Ijuka.app always wins startup.
            ...
            let server = Server(
                name: "ijuka",
                version: version,
                title: "Ijuka Flashcards & Active Retrieval",
            ...
            FileHandle.standardError.write(Data("ijuka-mcp: fatal: \(error)\n".utf8))
            exit(1)
        }
    }
    ...
    static func parseArguments() throws -> Options {
        ...
        while index < arguments.count {
            ...
            case "--config":
                ...
                environment["IJUKA_MCP_CONFIG"] = arguments[index]
            case "--help", "-h":
                printUsage()
                exit(0)
            case "--version":
                // stdout is protocol-clean here: no server was started.
                print("ijuka-mcp \(version)")
                exit(0)
            default:
    ...
    static func printUsage() {
        FileHandle.standardError.write(
            Data(
                """
                ijuka-mcp \(version) — Ijuka MCP server (macOS)
                usage: ijuka-mcp [--profile <id>] [--root <dir>] [--config <path>]
