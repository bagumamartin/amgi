...
enum ToolDispatcher {
    static func dispatch(
        ...
        do {
            ...
            return errorResult(
                "MCP settings are malformed; refusing the call until Ijuka rewrites them. (\(error.localizedDescription))"
            )
        }
        guard liveSettings.enabled else {
            return errorResult("The agent server is switched off in Ijuka settings.")
        }
        ...
        if tool.mutates {
            if liveSettings.blockWritesWhileAppRunning && AppRunningCheck.isIjukaRunning {
                return errorResult(
                    "writes are blocked while Ijuka.app is running (blockWritesWhileAppRunning=true). Quit the app or change the setting."
                )
            }
        }
        ...
        // can never block Ijuka.app from launching.
