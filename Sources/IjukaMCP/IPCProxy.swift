...
/// Forwards RPCs to Ijuka.app over the unix-socket bridge (bridged mode:
...
enum ProxyCaller {
    ...
    final class Session: @unchecked Sendable {
        ...
        init(socketPath: String, profileID: String, timeoutMs: Int32 = 120_000) throws {
            guard let fd = ProxyCaller.connectSocket(
                ...
                throw ToolError.blocked("could not connect to Ijuka.app")
            }
            self.fd = fd
    ...
    static func cleanStaleSocket(path: String) {
        ...
        guard !AppRunningCheck.isIjukaRunning else { return }
        var st = stat()
