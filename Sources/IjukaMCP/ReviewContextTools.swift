...
/// UI state rather than the collection. Asks Ijuka.app over the IPC
    ...
    static var tools: [IjukaTool] {
        [
            IjukaTool(
                ...
                description: """
                    What the user is looking at right now in the Ijuka app: the \
                    ...
                    Requires Ijuka to be open — call this first when the user \
                ...
                guard case .bridged(let session) = caller.kind else {
                    throw ToolError.blocked(
                        """
                        Ijuka.app is not running. The current-card context only exists while \
                        the app is open — ask the user to open Ijuka and start a review session. \
                        (Other Ijuka tools still work: they read the collection directly.)
                        """
                    )
                }
