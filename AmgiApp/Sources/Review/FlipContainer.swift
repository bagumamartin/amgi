...
struct FlipContainer<Content: View>: View {
    ...
    var body: some View {
        ...
        .onChange(of: showBack) { _, newValue in
            if newValue {
                // Diagnostic: time from the showAnswer flip to the first
                // runloop tick after the animation starts — captures how
                // long the back side's construction/layout delays commit.
                let startedAt = Date()
                withAnimation(.easeInOut(duration: 0.22)) { displayedBack = true }
                let afterAnimation = Date()
                DispatchQueue.main.async {
                    print("[FlipContainer] anim start→post-anim \(Int(afterAnimation.timeIntervalSince(startedAt) * 1000))ms, →tick \(Int(Date().timeIntervalSince(startedAt) * 1000))ms total")
                }
            } else {
