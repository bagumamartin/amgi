...
- **Model weights download from CDN on first launch (2026-09, supersedes the
  bundled-model rows above)**: `ModelAssetManager` (AmgiIcons actor) fetches
  `https://amgiassets.bagumamartin.com/manifest.json` (v1: 206MB
  ...
  system unzip on iOS), compiles via `MLModel.compileModel` into
  `Application Support/AmgiIcons/models/v<N>/` (backup-excluded), prunes old
  versions. Ships the `.mlpackage`, NOT `.mlmodelc`: compiler output is
  version-tied and zip-fragile. `TextEmbedder` lookup = installed dir →
  bundle `.mlmodelc` (dev leftover) → bundle `.mlpackage`→Caches → throw with
  graceful degradation (name-token icons, no semantic fallback). Manager calls
  `TextEmbedder.resetEngineForModelInstall()` post-install because load
  failures are memoized in `engineError`. App fires
  `ensureModelAvailable()` from a non-blocking `.task` in AmgiAppApp;
  `ModelAssetStatusView` is the Settings one-liner (progress/retry). Package
  builds with NO model present (`.copy mlmodelc` rule removed); engine tests
  `.enabled(if: isModelInstalled)` skip cleanly, manager tests assert the CDN
  contract offline. Tokenizer + IconEmbeddings.json stay bundled (few MB).
...
- **Engine moved to `AmgiEmbeddings/` sibling SPM (2026-09)**: AmgiIcons is
  ...
  live in AmgiEmbeddings (path dep from AmgiIcons + app).   Store renamed to
  `Application Support/AmgiEmbeddings/models`; `migrateLegacyStoreIfNeeded()`
  moves the unshipped `AmgiIcons/models` layout once. History preserved via
...
- **Consent + policy + toast (2026-09, app target `Sources/Sync/`)**: nothing
  downloads before explicit consent — `ModelDownloadCoordinator` (@Observable)
  runs the first-launch `.confirmationDialog` (Wi-Fi / cellular / offline
  variants, exact MB from a silent manifest fetch) from `ContentView` (so it
  only appears post-onboarding); the old blind launch `.task` in AmgiAppApp
  is gone. `ModelDownloadPreferences` (UserDefaults: consentGiven, wifiOnly
  default policy). `NetworkMonitor` (@Observable NWPathMonitor singleton)
  drives policy-aware auto-retry of `.failed` (`retryIfAllowed` on
  ...
  shares ONE bottom overlay with `SyncToast` (`combinedToastOverlay` — the
  old single `syncToastOverlay` extension is removed). MaintenanceView gains
  an AI Model section (status view + policy picker + confirmed delete).
  No system notifications — repo has zero UNUserNotification usage and a new
  permission prompt isn't worth it for a one-time download.
