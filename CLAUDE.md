...
- **New files need `xcodegen generate` before BuildProject sees them** — adding Swift files without regenerating produces "cannot find X in scope" storms that look like real errors. After regeneration, re-select the run destination (osascript snippet above).
- **Explicit-module build + new files + local SPM packages (Xcode 26.5)**: a brand-new source file importing a local path package can report "No such module" for that package while long-standing importers of the SAME package compile fine. Workaround: route the new file's calls through a shim in an existing importer (see `NoteEmbedderBridge.swift` ↔ `AmgiIcons`). Also: `ToolbarContentBuilder.buildBlock` caps at 10 items — consolidate with `ToolbarItemGroup`.
- **"swift compiler caching requires explicit module build" warnings** (Xcode 26.5): emitted once per target whose Swift compiles lose explicit modules due to C++ interop (AmgiReaderDictionary transitively pulls Cxx into AmgiApp). Cosmetic; no public setting disables it (`SWIFT_ENABLE_COMPILER_CACHING` probed inert). Do not chase — costs nothing at runtime.
- **Method-ID oracle**: `_backend_generated.py` in anki-upstream out/ holds literal `(service, method)` pairs; diff it after any upstream update before trusting catalog constants.
